...
namespace AutoCSer.Example.TcpInterfaceSimpleServer
{
    ...
    /// 接口继承与显示接口实现测试
    /// </summary>
    [AutoCSer.Net.TcpInternalSimpleServer.Server(Host = "127.0.0.1", Port = 13401)]
    ...
    class Inherit : IInherit
    {
        ...
        {
            using (AutoCSer.Net.TcpInternalSimpleServer.Server server = AutoCSer.Net.TcpInternalSimpleServer.Emit.Server<IInherit>.Create(new Inherit()))
            {
                if (server.IsListen)
                {
                    IInherit client = AutoCSer.Net.TcpInternalSimpleServer.Emit.Client<IInherit>.Create();
