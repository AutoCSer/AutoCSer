...
namespace AutoCSer.Example.TcpInterfaceSimpleServer
{
    ...
    /// ref / out 参数测试接口
    /// </summary>
    [AutoCSer.Net.TcpInternalSimpleServer.Server(Host = "127.0.0.1", Port = 13400, IsRemoteExpression = true)]
    ...
    class RefOut : IRefOut
    {
        ...
        internal static bool TestCase()
        {
            using (AutoCSer.Net.TcpInternalSimpleServer.Server server = AutoCSer.Net.TcpInternalSimpleServer.Emit.Server<IRefOut>.Create(new RefOut()))
            {
                if (server.IsListen)
                {
                    IRefOut client = AutoCSer.Net.TcpInternalSimpleServer.Emit.Client<IRefOut>.Create();
