    ...
    class CallBoxSerialize : AutoCSer.WebView.Call<CallBoxSerialize>
    {
        ...
        internal static bool TestCase()
        {
            using (WebClient webClient = new WebClient())
            {
                ...
                string json = webClient.UploadString(WebConfig.HttpDomain + @"CallBoxSerialize/Inc", "POST", AutoCSer.JsonSerializer.Serialize<int>(5));
                if (AutoCSer.JsonDeSerializer.DeSerialize<int>(json) != 6)
                {
                    return false;
                }
            }
