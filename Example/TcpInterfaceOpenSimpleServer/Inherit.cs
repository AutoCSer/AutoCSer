...
namespace AutoCSer.Example.TcpInterfaceOpenSimpleServer
{
    ...
    /// 接口继承与显示接口实现测试
    /// </summary>
    [AutoCSer.Net.TcpOpenSimpleServer.Server(Host = "127.0.0.1", Port = 13501)]
    ...
    class Inherit : IInherit
    {
        ...
        {
            using (AutoCSer.Net.TcpOpenSimpleServer.Server server = AutoCSer.Net.TcpOpenSimpleServer.Emit.Server<IInherit>.Create(new Inherit()))
            {
                if (server.IsListen)
                {
                    IInherit client = AutoCSer.Net.TcpOpenSimpleServer.Emit.Client<IInherit>.Create();
