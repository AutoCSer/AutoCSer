...
namespace AutoCSer.Example.TcpInterfaceOpenSimpleServer
{
    ...
    /// ref / out 参数测试接口
    /// </summary>
    [AutoCSer.Net.TcpOpenSimpleServer.Server(Host = "127.0.0.1", Port = 13500, IsRemoteExpression = true)]
    ...
    class RefOut : IRefOut
    {
        ...
        internal static bool TestCase()
        {
            using (AutoCSer.Net.TcpOpenSimpleServer.Server server = AutoCSer.Net.TcpOpenSimpleServer.Emit.Server<IRefOut>.Create(new RefOut()))
            {
                if (server.IsListen)
                {
                    IRefOut client = AutoCSer.Net.TcpOpenSimpleServer.Emit.Client<IRefOut>.Create();
