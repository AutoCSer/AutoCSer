...
namespace AutoCSer.Example.TcpInterfaceStreamServer
{
    ...
    /// </summary>
    [AutoCSer.Net.TcpInternalStreamServer.Server(Host = "127.0.0.1", Port = 13803, CommandIdentityEnmuType = typeof(ClientAwaiterCommand))]
    ...
    class ClientAwaiter : IClientAwaiterServer
    {
        ...
        {
            using (AutoCSer.Net.TcpInternalStreamServer.Server server = AutoCSer.Net.TcpInternalStreamServer.Emit.Server<IClientAwaiterServer>.Create(new ClientAwaiter()))
            {
                if (server.IsListen)
                {
                    IClientAwaiter client = AutoCSer.Net.TcpInternalStreamServer.Emit.Client<IClientAwaiter>.Create();
