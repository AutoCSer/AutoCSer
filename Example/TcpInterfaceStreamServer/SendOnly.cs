...
namespace AutoCSer.Example.TcpInterfaceStreamServer
{
    ...
    /// 仅发送请求测试
    /// </summary>
    [AutoCSer.Net.TcpInternalStreamServer.Server(Host = "127.0.0.1", Port = 13801)]
    ...
    class SendOnly : ISendOnly
    {
        ...
        {
            using (AutoCSer.Net.TcpInternalStreamServer.Server server = AutoCSer.Net.TcpInternalStreamServer.Emit.Server<ISendOnly>.Create(new SendOnly()))
            {
                if (server.IsListen)
                {
                    ISendOnly client = AutoCSer.Net.TcpInternalStreamServer.Emit.Client<ISendOnly>.Create();
