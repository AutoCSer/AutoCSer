...
namespace AutoCSer.Example.TcpInterfaceStreamServer
{
    ...
    /// 接口继承与显示接口实现测试
    /// </summary>
    [AutoCSer.Net.TcpInternalStreamServer.Server(Host = "127.0.0.1", Port = 13802)]
    ...
    class Inherit : IInherit
    {
        ...
        {
            using (AutoCSer.Net.TcpInternalStreamServer.Server server = AutoCSer.Net.TcpInternalStreamServer.Emit.Server<IInherit>.Create(new Inherit()))
            {
                if (server.IsListen)
                {
                    IInherit client = AutoCSer.Net.TcpInternalStreamServer.Emit.Client<IInherit>.Create();
