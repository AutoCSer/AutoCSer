...
namespace AutoCSer.Example.TcpInterfaceOpenStreamServer
{
    ...
    /// 接口继承与显示接口实现测试
    /// </summary>
    [AutoCSer.Net.TcpOpenStreamServer.Server(Host = "127.0.0.1", Port = 12704)]
    ...
    class Inherit : IInherit
    {
        ...
        {
            using (AutoCSer.Net.TcpOpenStreamServer.Server server = AutoCSer.Net.TcpOpenStreamServer.Emit.Server<IInherit>.Create(new Inherit()))
            {
                if (server.IsListen)
                {
                    IInherit client = AutoCSer.Net.TcpOpenStreamServer.Emit.Client<IInherit>.Create();
