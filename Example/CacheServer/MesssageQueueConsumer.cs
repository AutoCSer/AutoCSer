...
namespace AutoCSer.Example.CacheServer
{
    ...
    /// 消息队列测试
    /// </summary>
    internal class MesssageQueueConsumer
    {
        /// <summary>
        /// 消息队列测试
        /// </summary>
        ...
        {
            string name = "MesssageQueueConsumer";

            #region 删除名称为 MesssageQueueConsumer 的缓存数据，防止未处理数据对测试验证产生副作用
            client.RemoveDataStructure(name);
            #endregion

            #region 创建名称为 MesssageQueueConsumer 的 JSON 消息队列
            ...
            #region 创建消息队列的消费者
            using (AutoCSer.CacheServer.MessageQueue.Consumer<Json<int>, int> consumer = queue.CreateConsumer(onMessage, new AutoCSer.CacheServer.MessageQueue.ConsumerConfig { MemoryCacheNodeCount = 0 }))
            #endregion
            {
                #region 等待消息处理完毕
                wait.WaitOne();
