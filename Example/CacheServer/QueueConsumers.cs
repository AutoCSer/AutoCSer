...
namespace AutoCSer.Example.CacheServer
{
    ...
    internal class QueueConsumers
    {
        ...
        {
            string name = "QueueConsumers";

            #region 删除名称为 QueueConsumers 的缓存数据，防止未处理数据对测试验证产生副作用
            client.RemoveDataStructure(name);
            #endregion

            #region 创建名称为 QueueConsumers 的消息队列
            ...
            AutoCSer.CacheServer.MessageQueue.QueueConsumer<int> consumer = queue.CreateConsumer(0, onMessage, new AutoCSer.CacheServer.MessageQueue.Config.QueueConsumer { MemoryCacheNodeCount = 0 });
            #endregion
            using (consumer)
            {
                #region 等待消息处理完毕
