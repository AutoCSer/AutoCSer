...
namespace AutoCSer.Example.CacheServer
{
    ...
    internal class QueueConsumer
    {
        ...
        internal static bool TestCase(AutoCSer.CacheServer.Client client)
        {
            ...
            #region 删除名称为 QueueConsumer 的缓存数据，防止未处理数据对测试验证产生副作用
            ...
            #region 创建消息队列的消费者
            AutoCSer.CacheServer.MessageQueue.QueueConsumer<Json<int>, int> consumer = queue.CreateConsumer(onMessage, new AutoCSer.CacheServer.MessageQueue.Config.QueueConsumer { MemoryCacheNodeCount = 0 });
            #endregion
            using (consumer)
            {
                #region 等待消息处理完毕
