...
namespace AutoCSer.Web.SearchServer
{
    ...
    public sealed class Html
    {
            ...
            try
            {
                ...
                if (newHtml != null)
                {
                    ...
                    if (urls.TryGetValue(lowerUrl, out html) && html != Null)
                    {
                        ...
                        {
                            Searcher.SearchTaskQueue.CheckAdd(new Queue.Update(new DataKey { Type = DataType.HtmlTitle, Id = html.Id }, newHtml.Title, html.Title));
                            html.Title = newHtml.Title;
                        }
                        if (html.Text != newHtml.Text)
                        {
                            Searcher.SearchTaskQueue.CheckAdd(new Queue.Update(new DataKey { Type = DataType.HtmlBodyText, Id = html.Id }, newHtml.Text, html.Text));
                    ...
                    else
                    {
                        ...
                        Searcher.SearchTaskQueue.CheckAdd(new Queue.Append(new DataKey { Id = newHtml.Id, Type = DataType.HtmlTitle }, newHtml.Title));
                        Searcher.SearchTaskQueue.CheckAdd(new Queue.Append(new DataKey { Id = newHtml.Id, Type = DataType.HtmlBodyText }, newHtml.Text));
                        html = newHtml;
                    }
                    foreach (HtmlImage image in html.Images)
                    {
                        image.GetIndex(html.Id);
                        Searcher.SearchTaskQueue.CheckAdd(new Queue.Append(new DataKey { Id = image.Id, Type = DataType.HtmlImage }, image.Title));
                    }
                }
            }
        ...
        {
            Searcher.SearchTaskQueue.CheckAdd(new Queue.Delete(new DataKey { Type = DataType.HtmlTitle, Id = Id }, Title));
            Searcher.SearchTaskQueue.CheckAdd(new Queue.Delete(new DataKey { Type = DataType.HtmlBodyText, Id = Id }, Text));
        ...
        static Html()
        {
            ...
            foreach (FileInfo htmlFile in new DirectoryInfo(AutoCSer.Web.Config.Search.HtmlPath).GetFiles("*.html", SearchOption.AllDirectories))
            {
                ...
                if (html != null)
                {
                    ...
                    Searcher.SearchTaskQueue.CheckAdd(new Queue.Append(new DataKey { Id = html.Id, Type = DataType.HtmlTitle }, html.Title));
                    Searcher.SearchTaskQueue.CheckAdd(new Queue.Append(new DataKey { Id = html.Id, Type = DataType.HtmlBodyText }, html.Text));
                    foreach (HtmlImage image in html.Images)
                    {
                        image.GetIndex(html.Id);
                        Searcher.SearchTaskQueue.CheckAdd(new Queue.Append(new DataKey { Id = image.Id, Type = DataType.HtmlImage }, image.Title));
