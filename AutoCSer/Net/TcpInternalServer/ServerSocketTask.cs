...
namespace AutoCSer.Net.TcpInternalServer
{
    ...
    internal sealed class ServerSocketTask : AutoCSer.Threading.LinkTaskThread<ServerSocket>
    {
        ...
        {
            value.TaskTicks = ServerSocketTaskConfig.Default.Stopwatch.ElapsedTicks;
        ...
        {
            if (ServerSocketTaskConfig.Default.IsCheck(Task.currentTaskTicks))
            {
                if (isAllTask)
                {
                ...
                else
                {
                    try
                    {
                        Task = new ServerSocketTask(ServerSocketTaskConfig.Default.TaskTicks);
        ...
        {
            ServerSocketTaskConfig config = ServerSocketTaskConfig.Default;
