...
namespace AutoCSer.Net.TcpServer.Emit
{
    ...
    internal sealed partial class Method<attributeType, methodAttributeType, serverSocketSenderType>
    {
        ...
        internal struct ServerBuilder
        {
            ...
            /// <returns></returns>
            internal Type Build(Type type, ServerAttribute attribute, Type serverInterfaceType, Type serverCallType, Type[] constructorParameterTypes, Method<attributeType, methodAttributeType, serverSocketSenderType>[] methods, ServerCallQueue[] queues)
            {
                ...
                FieldBuilder[] queueFieldBuilders = queues.getArray(queue => typeBuilder.DefineField(queue.Name, queue.Type, FieldAttributes.Private | FieldAttributes.InitOnly));
                ...
                #endregion
                #region @QueueName = getServerCallQueue<@ServerCallQueueType.FullName>();
                foreach (ServerCallQueue queue in queues)
                {
                    constructorGenerator.Emit(OpCodes.Ldarg_0);
                    constructorGenerator.Emit(OpCodes.Ldarg_0);
                    constructorGenerator.call(Metadata.GetServerCallQueueMethod.MakeGenericMethod(queue.ParameterType));
                    constructorGenerator.Emit(OpCodes.Stfld, queueFieldBuilders[queue.Index]);
                }
                #endregion
                ...
                foreach (Method<attributeType, methodAttributeType, serverSocketSenderType> method in methods)
                {
                    if (method != null)
                    {
                        ...
                        FieldBuilder asynchronousCallbackFieldBuilder = null;
                        if (method.IsMethodServerCall)
                        {
                            ...
                            if (method.IsAsynchronousCallback)
                            {
                                ...
                                callGenerator.Emit(OpCodes.Ldfld, Metadata.ServerCallServerValueField);
                                ...
                                foreach (ParameterInfo parameter in parameters)
                                {
                                    ...
                                    {
                                        callGenerator.Emit(OpCodes.Ldarg_0);
                                        callGenerator.Emit(OpCodes.Ldflda, inputParameterField);
                                        callGenerator.Emit(OpCodes.Ldfld, method.ParameterType.GetField(parameter.Name));
                                    }
                                    ...
                                    else callGenerator.Emit(OpCodes.Ldnull);
                                }
                                callGenerator.Emit(OpCodes.Ldarg_0);
                                callGenerator.Emit(OpCodes.Ldfld, asynchronousCallbackFieldBuilder);
                                callGenerator.call(method.MethodInfo);
                                #endregion
                                if(method.IsKeepCallback) callGenerator.Emit(OpCodes.Pop);
                        ...
                        else
                        {
                            ...
                            if (method.IsAsynchronousCallback)
                            {
                                ...
                                if (method.IsMethodServerCall)
                                {
                                    #region @MethodStreamName serverCall = @MethodStreamName.PopNew();
                                    if (method.ParameterType == null) methodGenerator.call(Metadata.ServerCallPopNewMethod.MakeGenericMethod(serverCallTypeBuilder));
                                    else methodGenerator.call(serverCallType.MakeGenericType(method.ParameterType.Type).GetMethod("PopNew", BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly).MakeGenericMethod(serverCallTypeBuilder));
                                    methodGenerator.Emit(OpCodes.Dup);
                                    #endregion
                                    #region serverCall.AsynchronousCallback = sender.GetCallback<@OutputParameterTypeName, @MethodReturnType.FullName>(@MethodIdentityCommand, ref outputParameter);
                                    if (method.ReturnType == typeof(void))
                                    {
                                        methodGenerator.Emit(OpCodes.Ldarg_1);
                                    ...
                                    #endregion
                                    #region serverCall.Call(sender, _value_, @ServerTask, ref inputParameter);
                                    methodGenerator.Emit(OpCodes.Ldarg_1);
                                    ...
                                    {
                                        #region @QueueName.Get(sender, ref inputParameter.AutoCSerTcpQueueKey)
                                        methodGenerator.Emit(OpCodes.Ldarg_0);
                                        ...
                                        #endregion
                                        methodGenerator.Emit(OpCodes.Ldloca_S, parameterLocalBuilder);
                                        methodGenerator.call(serverCallTypeBuilder.GetMethod("CallQueue", BindingFlags.Instance | BindingFlags.Public));
                                    }
                                    #endregion
                                }
                                else
                            ...
                            {
                                #region @MethodStreamName.PopNew().Call(sender, _value_, @ServerTask, ref inputParameter);
                                if (method.ParameterType == null) methodGenerator.call(Metadata.ServerCallPopNewMethod.MakeGenericMethod(serverCallTypeBuilder));
                                else methodGenerator.call(serverCallType.MakeGenericType(method.ParameterType.Type).GetMethod("PopNew", BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly).MakeGenericMethod(serverCallTypeBuilder));
