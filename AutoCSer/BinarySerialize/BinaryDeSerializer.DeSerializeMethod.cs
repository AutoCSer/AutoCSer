    ...
    public unsafe sealed partial class BinaryDeSerializer
    {
            ...
            if ((length & 1) == 0)
            {
                if (length != 0)
                {
                    ...
                    if (dataLength <= (int)(End - Read))
                    {
                        ...
                        {
                            fixed (char* valueFixed = value) AutoCSer.Memory.Common.CopyNotNull(Read + sizeof(int), valueFixed, length);
                            Read += dataLength;
                        }
                        return;
                    }
        ...
        public unsafe void CallDeSerialize(ref SubString value)
        {
            ...
            else
            {
                ...
                if (((lengthSize + length + (3 + sizeof(int))) & (int.MaxValue - 3)) <= (int)(End - Read))
                {
                    value.Set(AutoCSer.Extensions.StringExtension.FastAllocateString(length), 0, length);
        ...
        public void CallDeSerialize(ref Type value)
        {
            if (CheckPoint(ref value))
            {
                ...
                TypeDeSerializer<AutoCSer.Reflection.RemoteType>.GetDeSerializer(GlobalVersion).MemberDeSerialize(this, ref remoteType);
