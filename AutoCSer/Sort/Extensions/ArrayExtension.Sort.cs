...
namespace AutoCSer.Extensions
{
    ...
    /// 数组扩展操作
    /// </summary>
    public static partial class ArrayExtensionSort
    {
        /// <summary>
        /// 范围排序
        /// </summary>
        ...
        {
            if (values == null) return new SubArray<valueType>();
            if (comparer == null) throw new ArgumentNullException();
            if (count > 0)
            {
        ...
        {
            uint sqrtMod;
            int length = Math.Min(Math.Max(count << 2, count + (int)NumberExtension.sqrt((uint)values.Length, out sqrtMod)), values.Length);
