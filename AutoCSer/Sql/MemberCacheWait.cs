...
namespace AutoCSer.Sql
{
    ...
    /// 成员扩展缓存初始化依赖类型加载
    /// </summary>
    internal sealed class MemberCacheWait
    {
        /// <summary>
        /// 成员扩展缓存初始化依赖类型加载等待事件
        /// </summary>
        ...
        /// 依赖当前缓存初始化的类型集合
        /// </summary>
        private LeftArray<MemberCacheWait> waits;
        /// <summary>
        /// 成员扩展缓存初始化依赖类型加载
        /// </summary>
        /// <param name="wait">依赖当前缓存初始化的类型</param>
        private MemberCacheWait(MemberCacheWait wait)
        {
            this.wait.Set(0);
            waits.Add(wait);
        }
        ...
        private void set(HashSet<Type> cacheTypes)
        {
            ...
            foreach (Type cacheType in cacheTypes)
            {
                ...
                else loadTypes.Add(cacheType, new MemberCacheWait(this));
            }
        ...
        /// 数据完成类型加载集合
        /// </summary>
        private static readonly Dictionary<Type, MemberCacheWait> loadTypes = DictionaryCreator.CreateOnly<Type, MemberCacheWait>();
        /// <summary>
        /// 数据完成类型加载集合访问锁
        /// </summary>
        ...
        internal static void Load(Type tableType, HashSet<Type> cacheTypes)
        {
            ...
            try
            {
                ...
                else loadTypes.Add(tableType, wait = new MemberCacheWait(cacheTypes));
            }
