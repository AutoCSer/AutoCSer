...
namespace AutoCSer.Extensions
{
    ...
    /// 字符串相关操作
    /// </summary>
    internal unsafe static partial class StringExtensionHtmlNode
    {
        ///// <summary>
        ///// 字符查找
        ///// </summary>
        ...
        internal static bool EqualCase(this SubString left, string right)
        {
            ...
            if (left.Length == right.Length)
            {
                if (left.Length != 0)
                {
                    ...
                    fixed (char* leftFixed = left.GetFixedBuffer(), rightFixed = right)
                    {
                        return AutoCSer.Extensions.StringExtension.equalCaseNotNull(leftFixed + left.Start, rightFixed, left.Length);
                    }
                }
