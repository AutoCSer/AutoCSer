...
namespace AutoCSer.CodeGenerator.TemplateGenerator
{
    ...
    /// TCP 服务代码生成
    /// </summary>
    internal sealed partial class TcpOpenStreamServer : TcpStreamServer
    {
        /// <summary>
        /// TCP 服务代码生成
        /// </summary>
        [Generator(Name = "TCP 开放应答流服务", DependType = typeof(CSharper), IsAuto = true)]
        internal sealed partial class Generator : Generator<AutoCSer.Net.TcpOpenStreamServer.ServerAttribute, AutoCSer.Net.TcpOpenStreamServer.MethodAttribute, AutoCSer.Net.TcpOpenStreamServer.ServerSocketSender>
        {
            /// <summary>
            /// 服务类名称
            /// </summary>
            ...
            {
                get { return typeof(AutoCSer.Net.TcpOpenStreamServer.TimeVerifyServer).IsAssignableFrom(Type); }
            }
            /// <summary>
            /// 是否生成客户端代码
            /// </summary>
...
#else
                    return typeof(AutoCSer.Net.TcpServer.ISetTcpServer<AutoCSer.Net.TcpOpenStreamServer.Server, AutoCSer.Net.TcpOpenStreamServer.ServerAttribute>).IsAssignableFrom(Type.Type)
#endif
                        || Type.Type.GetMethod("SetTcpServer", BindingFlags.Instance | BindingFlags.Public, null, new Type[] { typeof(AutoCSer.Net.TcpOpenStreamServer.Server) }, null) != null;
                }
            }
            /// <summary>
            /// 安装下一个类型
            ...
            protected unsafe override void nextCreate()
            {
                ...
                {
                    LeftArray<TcpMethod> methodArray = new LeftArray<TcpMethod>(Metadata.MethodIndex.GetMethods<AutoCSer.Net.TcpOpenStreamServer.MethodAttribute>(Type, Attribute.MemberFilters, false, Attribute.IsAttribute, Attribute.IsBaseTypeAttribute)
                    ...
                    foreach (MemberIndexInfo member in MemberIndexGroup.Get<AutoCSer.Net.TcpOpenStreamServer.MethodAttribute>(Type, Attribute.MemberFilters, false, Attribute.IsAttribute, Attribute.IsBaseTypeAttribute))
                    {
                        if (member.IsField)
                        {
                    ...
                    if (ServiceAttribute.IsSegmentation)
                    {
                        ...
                        string fileName = AutoParameter.ProjectPath + "{" + AutoParameter.DefaultNamespace + "}.TcpOpenStreamServer." + ServerName + ".Client.cs";
                        ...
                        if (Coder.WriteFile(fileName, clientCode))
                        {
                            ...
                            {
                                string copyFileName = ServiceAttribute.ClientSegmentationCopyPath + "{" + AutoParameter.DefaultNamespace + "}.TcpOpenStreamServer." + ServerName + ".Client.cs";
