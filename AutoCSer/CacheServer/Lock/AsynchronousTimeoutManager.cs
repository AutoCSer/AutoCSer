    ...
    public sealed class AsynchronousTimeoutManager : ManagerBase
    {
        ...
        private void enter(AutoCSer.Net.TcpServer.ReturnValue<ReturnParameter> returnParameter)
        {
            if (Interlocked.CompareExchange(ref enterLock, 1, 0) == 0)
            {
                ...
                else onEnter(new ReturnValue<AsynchronousTimeoutManager> { Type = returnParameter.Value.Type, TcpReturnType = returnParameter.Type });
            }
        ...
        internal void Enter(uint timeoutMilliseconds)
        {
            ...
            AutoCSer.Threading.TimerTask.Default.Add(enterTimeout, Date.NowTime.Set().AddTicks(timeoutMilliseconds * Date.MillisecondTicks));
        }
        /// <summary>
        /// 申请锁
        /// </summary>
