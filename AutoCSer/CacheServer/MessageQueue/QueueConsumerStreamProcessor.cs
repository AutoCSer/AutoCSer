...
namespace AutoCSer.CacheServer.MessageQueue
{
    ...
    internal sealed class QueueConsumerStreamProcessor<valueType> : Abstract.QueueConsumerStreamProcessor
    {
        /// <summary>
        /// 消息处理委托
        /// </summary>
        ...
        private void onGetDequeueIdentity(AutoCSer.Net.TcpServer.ReturnValue<ReturnParameter> identity)
        {
            ...
            {
                if (isIdentity == 0 && consumer.IsProcessor(this)) AutoCSer.Threading.TimerTask.Default.Add(Start, Date.NowTime.Now.AddTicks(Date.SecondTicks));
            }
        }
        /// <summary>
        /// 获取当前读取数据标识
