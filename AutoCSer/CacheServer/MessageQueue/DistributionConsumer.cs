...
namespace AutoCSer.CacheServer.MessageQueue
{
        ...
        where nodeType : DataStructure.Abstract.Node
    {
        ...
        /// <param name="log">日志处理</param>
        public DistributionConsumer(DataStructure.MessageQueue.Distributor<nodeType> node, Action<valueType> onMessage, DistributionConsumerConfig config, ValueData.GetData<valueType> getValue, AutoCSer.ILog log = null) : base(node, config, log, getValue)
        {
            if (onMessage == null) throw new ArgumentNullException();
            this.onMessage = onMessage;
            setCheckSocketVersion();
