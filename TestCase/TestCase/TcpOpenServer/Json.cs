...
namespace AutoCSer.TestCase.TcpOpenServer
{
    ...
    /// </summary>
    [AutoCSer.Net.TcpOpenServer.Server(Host = "127.0.0.1", Port = (int)ServerPort.TcpOpenServer_Json, IsSegmentation = false, IsJsonSerialize = true)]
    internal partial class Json : TcpInternalServer.Json
    {
        /// <summary>
        /// TCP 服务 JSON 序列化测试
        /// </summary>
        ...
        internal new static bool TestCase()
        {
            ...
            using (Json.TcpOpenServer server = new Json.TcpOpenServer())
            {
                if (server.IsListen)
                {
                    using (Json.TcpOpenClient client = new Json.TcpOpenClient())
                    {
                        incValue = 0;
                        client.Inc();
