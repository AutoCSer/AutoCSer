...
namespace AutoCSer.TestCase.TcpOpenSimpleServer
{
    ...
    /// </summary>
    [AutoCSer.Net.TcpOpenSimpleServer.Server(Host = "127.0.0.1", Port = (int)ServerPort.TcpOpenSimpleServer_Json, IsSegmentation = false, IsJsonSerialize = true)]
    internal partial class Json : TcpInternalSimpleServer.Json
    {
        /// <summary>
        /// TCP 服务 JSON 序列化测试
        /// </summary>
        ...
        {
#if !NoAutoCSer
            using (Json.TcpOpenSimpleServer server = new Json.TcpOpenSimpleServer())
            {
                if (server.IsListen)
                {
                    using (Json.TcpOpenSimpleClient client = new Json.TcpOpenSimpleClient())
                    {
                        incValue = 0;
                        client.Inc();
