...
namespace AutoCSer.TestCase.TcpInternalStreamServer
{
    ...
    /// </summary>
    [AutoCSer.Net.TcpInternalStreamServer.Server(Host = "127.0.0.1", Port = (int)ServerPort.TcpInternalStreamServer_Json, IsServer = true, IsRememberCommand = false, IsJsonSerialize = true)]
    internal partial class Json
    {
        ...
        internal static bool TestCase()
        {
            ...
            using (Json.TcpInternalStreamServer server = new Json.TcpInternalStreamServer())
            {
                if (server.IsListen)
                {
                    using (Json.TcpInternalStreamClient client = new Json.TcpInternalStreamClient())
                    {
                        incValue = 0;
                        client.Inc();
