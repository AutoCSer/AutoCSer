...
namespace AutoCSer.TestCase.TcpInternalStreamServer.Emit
{
    ...
    class Server : IServer
    {
        ...
        {
            using (AutoCSer.Net.TcpInternalStreamServer.Server server = AutoCSer.Net.TcpInternalStreamServer.Emit.Server<IServer>.Create(new Server()))
            {
                if (server.IsListen)
                {
                    return testCase(AutoCSer.Net.TcpInternalStreamServer.Emit.Client<IServer>.Create());
                }
            }
            return false;
        }
        ...
        {
            IServer client = AutoCSer.Net.TcpInternalStreamServer.Emit.Client<IServer>.Create();
            using (client as IDisposable)
            {
                long value;
