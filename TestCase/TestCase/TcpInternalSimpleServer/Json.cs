...
namespace AutoCSer.TestCase.TcpInternalSimpleServer
{
    ...
    /// </summary>
    [AutoCSer.Net.TcpInternalSimpleServer.Server(Host = "127.0.0.1", Port = (int)ServerPort.TcpInternalSimpleServer_Json, IsServer = true, IsRememberCommand = false, IsJsonSerialize = true)]
    internal partial class Json
    {
        ...
        {
#if !NoAutoCSer
            using (Json.TcpInternalSimpleServer server = new Json.TcpInternalSimpleServer())
            {
                if (server.IsListen)
                {
                    using (Json.TcpInternalSimpleClient client = new Json.TcpInternalSimpleClient())
                    {
                        incValue = 0;
                        client.Inc();
